...
window.grid_gallery_builder = (panel, editor)->
  tpl = $('<div><table class="table table-hover">' +
      '<thead><th></th><th>'+I18n("mvm_js.gallery.title")+'</th><th class="hidden"></th></thead>' +
      '<tbody></tbody>' +
      '</table>'+
  ...
  tpl.find(".add_item").click(->
    show_form(add_item(I18n("mvm_js.gallery.title_sample")))
    return false;
  )
  tpl.on("click", "a.edit_item", ->
    ...
    return false if(!confirm(I18n("mvm_js.gallery.confirm_del")))
  ...
  add_item = (title, text)->
    ...
    tr = $("<tr><td><i class='fa fa-arrows' style='cursor: move;'></i></td><td class='name'></td><td class='text-right'>"+fields+" <a href='#' class='edit_item' title='"+I18n("mvm_js.gallery.edit")+"'><i class='fa fa-pencil'></i></a> <a href='#' class='del_item' title='"+I18n("mvm_js.gallery.delete")+"'><i class='fa fa-trash'></i></a></td></tr>")
  ...
  show_form = (tr)->
    form = $('<form>'+
            '<div class="form-group"><label class="control-label">'+I18n("mvm_js.gallery.title")+'</label><input class="form-control name"></div>'+
            '<div class="form-group"><label class="control-label">'+I18n("mvm_js.gallery.media")+'</label>
    ...
    open_modal({id: 'cama_editor_modal2', title: I18n("mvm_js.gallery.form_title"), type: "primary", modal_size: "modal-lg", modal_settings: { keyboard: false, backdrop: "static" }, content: form, callback: form_callback, on_submit: submit_form_callback })
  ...
  open_modal({title: I18n("mvm_js.gallery.gallery_panel"), modal_size: "modal-lg", modal_settings: { keyboard: false, backdrop: "static" }, content: tpl, on_submit: submit_callback })
