...
window.grid_tab_builder = (panel, editor)->
  ...
  tpl = $('<div><table class="table table-hover">' +
      '<thead><th></th><th>'+I18n.t("mvm_js.tabs.title")+'</th><th class="hidden">'+I18n.t("mvm_js.tabs.content")+'</th><th></th></thead>' +
      '<tbody></tbody>' +
      '</table>'+
  ...
  tpl.find(".add_item").click(->
    show_form(add_item(I18n.t("mvm_js.tabs.title_sample")))
    return false;
  )
  tpl.on("click", "a.edit_item", ->
    ...
    return false if(!confirm(I18n.t("mvm_js.tabs.confirm_del")))
  ...
  open_modal({title: I18n.t("mvm_js.tabs.tab_panel"), modal_size: "modal-lg", modal_settings: { keyboard: false, backdrop: "static" }, content: tpl, on_submit: submit_callback })
